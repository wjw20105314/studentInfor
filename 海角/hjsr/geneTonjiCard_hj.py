...
try:
    ...
    config.read('properties_tonji_hj.conf', encoding='utf-8-sig')
    ...
    file_name4 = config['fileCofig']['file_name4']  # 户措施
    year2018 = config['fileCofig']['year2018']
    year2019 = config['fileCofig']['year2019']
    year2020 = config['fileCofig']['year2020']
    # 1.读取措施年度收支统计-18
    # 按每一户存储
    context1 = {}
    ...
    for i in range(2, ws4.max_row + 1):
        ...
        if str(cell_4f) not in (str(year2018),str(year2019),str(year2020)):
            continue
        # 获取村，户码，户主姓名
        cell_4c = ws4['C' + str(i)].value
        cell_4h = ws4['H' + str(i)].value
        cell_4g = ws4['G' + str(i)].value
        key4 = cell_4c + cell_4h + cell_4g

        # 人数
        cell_4j = int(ws4['J' + str(i)].value)
        # 判断key是否存在，不存在则新加
        if key4 not in context4.keys():
            # 放入户主
            hu_4dict = {}
            if str(cell_4f) == str(year2018):
                hu_4dict['ren18'] = cell_4j
            elif str(cell_4f) == str(year2019):
                hu_4dict['ren19'] = cell_4j
            elif str(cell_4f) == str(year2020):
                hu_4dict['ren20'] = cell_4j

            context4[key4] = hu_4dict
        else:
            # 存在则取出处理
            hu_4dict = context4[key4]
            if str(cell_4f) == str(year2018):
                hu_4dict['ren18'] = cell_4j
            elif str(cell_4f) == str(year2019):
                hu_4dict['ren19'] = cell_4j
    ...
    for key in context3.keys():
        ...
        # 取出每一户的信息，生成一份word
        word = dict_3
        doc = DocxTemplate("model_tongji_hj.docx")  # 模板文档
        cunName = dict_3['cun']
