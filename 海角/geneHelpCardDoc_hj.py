...
try:
    ...
    file_name8 = config['fileCofig']['file_name8']  # 财产性收入明细
    ...
    # 按每一户存储
    context8 = {}
    wb8 = openpyxl.load_workbook(file_name8)
    ws8 = wb8.active
    # 从第2行开始遍历
    for i in range(2, ws8.max_row + 1):

        # 获取村，户码，户主姓名
        cell_8a = ws8['A' + str(i)].value
        cell_8b = ws8['B' + str(i)].value
        cell_8c = ws8['C' + str(i)].value
        key8 = cell_8a + cell_8b + cell_8c
        # 添加人员信息
        hu_8dict = {}
        # 取户资产收入明细
        cell_8e = ws8['e' + str(i)].value
        hu_8dict['ccxsr'] = cell_8e
        context8[key8] = hu_8dict
    ...
    for key in context2.keys():
        ...
        if key in context8.keys():
            dict_8 = context8[key]
            dict_2.update(dict_8)
        ...
        # 取出每一户的信息，生成一份word
        word = dict_2
        doc = DocxTemplate("model_hj.docx")  # 模板文档
        cunName = dict_2['cun']
        path = ".\\" + cunName + "村帮扶明白卡\\"
